"""
From https://github.com/YyzHarry/imbalanced-regression
"""
...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, bucket_num=50, bucket_start=0, start_update=0, start_smooth=1,
        ...
        self.kernel_window = self._get_kernel_window(kernel, ks, sigma).to(device)
        self.half_ks = (ks - 1) // 2
    ...
    def update_running_stats(self, features, buckets, epoch):
        if epoch < self.epoch:
            return

        assert self.feature_dim == features.size(1), "Input feature dimension is not aligned!"
        assert features.size(0) == buckets.size(0), "Dimensions of features and labels are not aligned!"

        for bucket in torch.unique(buckets):
            curr_feats = features[buckets == bucket]
        ...
        for bucket in range(self.bucket_start, self.bucket_num):
            if bucket not in torch.unique(buckets):
