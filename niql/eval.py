"""
Adapted from https://github.com/Morphlng/MARLlib/blob/main/examples/eval.py
"""
...
import niql.envs
from niql.algo import NIQL_TRAINERS
from niql.models import *  # noqa
from niql import config as NIQL_CONFIG
...
def form_algo_dict() -> dict[str, tuple[str, Trainer]]:
    ...
    # add niql trainers
    trainers_dict.update(NIQL_TRAINERS)
...
def update_config(config: dict):
    ...
    try:
        ...
        env = niql.envs.make_matrix_game_env()
    env_instance, env_info = env
    algorithm = dotdict({"name": algo_name, "algo_type": ALGO_DICT[algo_name][0]})
    model_instance, model_info = marl.build_model(env, algorithm, model_preference=model_arch_args)
    if model_arch_args.get('custom_model'):
        model_instance, model_info = (eval(model_arch_args['custom_model']), model_info)
    ...
    config["model"]["lstm_cell_size"] = None
    ...
    if share_policy == "all":
        ...
        policy_ids = list(policies.keys())
        policy_mapping_fn = (lambda agt_id: policy_ids[agent_name_ls.index(agt_id)])
    ...
    # custom props
    space_obs = env_info["space_obs"]["obs"]
    setattr(space_obs, 'original_space', copy.deepcopy(space_obs))
    config["obs_space"] = space_obs
    action_space = env_info["space_act"]
    config["act_space"] = GymTuple([action_space])
    config.update(NIQL_CONFIG.COOP_MATRIX["algo_parameters"]["algo_args"])
...
def load_model(model_config: dict) -> Checkpoint:
    ...
    trainer_tuple = ALGO_DICT[algo]
    if len(trainer_tuple) == 3:
        default_config = trainer_tuple[2]
        default_config.update(params)
        params = default_config
    trainer_class = ALGO_DICT[algo][1].with_updates(
        name=algo.upper(),
        default_config=params,
    )
    trainer = trainer_class(logger_creator=lambda config: NullLogger(config))
